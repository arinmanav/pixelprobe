...
class ROISelector:
    ...
    def _setup_rectangle_selector(self):
        """Setup rectangle selection with minimum 2x1 or 1x2 pixel requirement"""
        self.current_selector = RectangleSelector(
            self.subplot,
            self._on_rectangle_select,
            useblit=True,
            button=[1],  # Only left mouse button
            minspanx=0.5,  # Allow 1-pixel width selections
            minspany=0.5,  # Allow 1-pixel height selections  
            spancoords='data',
            interactive=True,
    ...
    def _on_rectangle_select(self, eclick, erelease):
        ...
        # Rectangle ROI blocks only 1x1 pixels (single pixels use Point ROI)
        if width == 1 and height == 1:
            self._update_status("Rectangle ROI requires minimum 2×1 or 1×2 pixels. Use Point ROI for single pixels.")
            return
    ...
    def _visualize_roi(self, roi: ROI):
        ...
        if roi.roi_type == ROIType.RECTANGLE:
            ...
            y_max_boundary = y + h - 0.5
            
            # Draw the frame exactly around the selected pixel boundaries
            selected_rect = patches.Rectangle(
                (x_min_boundary, y_min_boundary), 
            ...
            # Add a light fill to show the selected pixel area
            ...
            corners = [
                ...
                (x_max_boundary, y_max_boundary),  # top-right
                (x_min_boundary, y_max_boundary)   # top-left
            ]
            ...
            # Draw frame around the single pixel
            pixel_frame = patches.Rectangle(
                (x - 0.5, y - 0.5), 1, 1,
                fill=False,
                edgecolor=roi.color,
                linewidth=2.0,
            ...
            self.subplot.add_patch(pixel_frame)
            
            # Light fill
            pixel_fill = patches.Rectangle(
                (x - 0.5, y - 0.5), 1, 1,
                fill=True,
                facecolor=roi.color,
                alpha=0.1,
            ...
            self.subplot.add_patch(pixel_fill)
            ...
            # Visualize multiple points, each with its own pixel frame
